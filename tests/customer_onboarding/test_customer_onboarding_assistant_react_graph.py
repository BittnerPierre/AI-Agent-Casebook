...
def assistant(config):
    ...
    assistant = create_customer_onboarding_assistant_as_react_graph(model_name=default_model)
...
def test_customer_onboarding_assistant_faq(assistant):
    ...
    ai_msg_7 = assistant.invoke(input={'messages': human_msg7},
                                config={'configurable': {'session_id': session_id, 'thread_id': session_id}})
    # BASIC ANSWER
    print(ai_msg_7)
...
def test_customer_onboarding_assistant_problem(assistant):
    ...
    ai_msg_5 = assistant.invoke(input={'messages': human_msg5},
                                config={'configurable': {'session_id': session_id, 'thread_id': session_id}})
